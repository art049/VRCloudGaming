...
<<<<<<< HEAD
			...
			if (this.mc.theWorld != null) {
				...
				List<Entity> list = this.mc.theWorld.getEntitiesInAABBexcluding(entity,
						entity.getEntityBoundingBox()
								.addCoord(vec3d1.xCoord * d0, vec3d1.yCoord * d0, vec3d1.zCoord * d0)
	...
	private float getFOVModifier(float partialTicks, boolean useFOVSetting) {
		if (this.debugView) {
			...
			IBlockState iblockstate = ActiveRenderInfo.getBlockStateAtEntityViewpoint(this.mc.theWorld, entity,
					partialTicks);
	...
	private void orientCamera(float partialTicks) {
		...
		if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).isPlayerSleeping()) {
			...
			double d3 = (double) (this.thirdPersonDistancePrev
			...
			if (this.mc.gameSettings.debugCamEnable) {
				...
				for (int i = 0; i < 8; ++i) {
					...
					f4 = f4 * 0.1F;
					f5 = f5 * 0.1F;
					RayTraceResult raytraceresult = this.mc.theWorld
	...
	private void setupCameraTransform(float partialTicks, int pass) {
		...
		if (pass == 2)
			...
			Project.gluPerspective(this.getFOVModifier(partialTicks, true),
					(float) this.mc.displayWidth / (float) this.mc.displayHeight / 2, 0.05F,
					this.farPlaneDistance * MathHelper.SQRT_2);
		// END VRCG
	...
	public void renderWorld(float partialTicks, long finishTimeNano) {
		...
		if (this.mc.gameSettings.anaglyph) {
			...
			anaglyphField = 1;
			// System.out.println(" fin pass 0
			// "+GlStateManager.colorMaskState.alpha);
			GlStateManager.colorMask(false, false, false, false);// TODO
																	// :Pourquoi,
																	// je sais
																	// pas
			this.renderWorldPass(1, partialTicks, finishTimeNano);
			// System.out.println(" fin pass 1 " +
			// GlStateManager.colorMaskState.alpha);
			GlStateManager.colorMask(true, true, true, true);
			// END VRCG
		} else {
		...
		if (f != 0.0F) {
			...
			if (j > 0 && this.random.nextInt(3) < this.rainSoundCounter++) {
				...
				if (d1 > (double) (blockpos.getY() + 1) && world.getPrecipitationHeight(blockpos).getY() > MathHelper
					...
					this.mc.theWorld.playSound(d0, d1, d2, SoundEvents.WEATHER_RAIN_ABOVE, SoundCategory.WEATHER, 0.1F,
							0.5F, false);
				} else {
					this.mc.theWorld.playSound(d0, d1, d2, SoundEvents.WEATHER_RAIN, SoundCategory.WEATHER, 0.2F, 1.0F,
							false);
				}
			}
		}
	...
	private void updateFogColor(float partialTicks) {
		...
		if (this.mc.gameSettings.renderDistanceChunks >= 4) {
			double d0 = -1.0D;
			Vec3d vec3d2 = MathHelper.sin(world.getCelestialAngleRadians(partialTicks)) > 0.0F
					? new Vec3d(d0, 0.0D, 0.0D) : new Vec3d(1.0D, 0.0D, 0.0D);
		...
		if (this.mc.gameSettings.anaglyph) {
			// BEGIN VRCG
			// float f16 = (this.fogColorRed * 30.0F + this.fogColorGreen *
			// 59.0F + this.fogColorBlue * 11.0F) / 100.0F;
			// float f17 = (this.fogColorRed * 30.0F + this.fogColorGreen *
			// 70.0F) / 100.0F;
			// float f7 = (this.fogColorRed * 30.0F + this.fogColorBlue * 70.0F)
			// / 100.0F;
   ...
   private void addRainParticles() {
      ...
      if(f != 0.0F) {
         ...
         for(int l = 0; l < k; ++l) {
            ...
            BlockPos blockpos2 = blockpos1.down();
            IBlockState iblockstate = world.getBlockState(blockpos2);
            if(blockpos1.getY() <= blockpos.getY() + i && blockpos1.getY() >= blockpos.getY() - i && biome.canRain() && biome.getFloatTemperature(blockpos1) >= 0.15F) {
               double d3 = this.random.nextDouble();
               double d4 = this.random.nextDouble();
               AxisAlignedBB axisalignedbb = iblockstate.getBoundingBox(world, blockpos2);
               if(iblockstate.getMaterial() == Material.LAVA) {
