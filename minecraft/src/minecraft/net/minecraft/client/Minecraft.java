...
public class Minecraft implements IThreadListener, ISnooperInfo {
	...
	private void updateDisplayMode() throws LWJGLException {
		...
		if (!set.contains(displaymode) && Util.getOSType() == Util.EnumOS.OSX) {
			label53: for (DisplayMode displaymode1 : MAC_DISPLAY_MODES) {
				boolean flag = true;
	...
	private void drawSplashScreen(TextureManager textureManagerInstance) throws LWJGLException {
		...
		try {
			...
			LOGGER.error((String) ("Unable to load logo: " + LOCATION_MOJANG_PNG), (Throwable) ioexception);
		} finally {
			IOUtils.closeQuietly(inputstream);
		}
		...
		if (list != null && !list.isEmpty()) {
			...
			if (keyCount == 0) {
				...
				if (keyCount < list.size()
						&& !((Profiler.Result) list.get(keyCount)).profilerName.equals("unspecified")) {
	...
	private void displayDebugInfo(long elapsedTicksTime) {
		if (this.mcProfiler.profilingEnabled) {
			...
			vertexbuffer.pos((double) ((float) j - (float) i * 1.1F), (double) (k + i * 2), 0.0D).color(200, 0, 0, 0)
					.endVertex();
			vertexbuffer.pos((double) ((float) j + (float) i * 1.1F), (double) (k + i * 2), 0.0D).color(200, 0, 0, 0)
					.endVertex();
			vertexbuffer
			...
			if (!this.thePlayer.isRowingBoat()) {
				for (EnumHand enumhand : EnumHand.values()) {
					...
					if (this.objectMouseOver == null) {
						...
						switch (this.objectMouseOver.typeOfHit) {
						case ENTITY:
							if (this.playerController.interactWithEntity(this.thePlayer, this.objectMouseOver.entityHit,
									this.objectMouseOver, this.thePlayer.getHeldItem(enumhand),
									enumhand) == EnumActionResult.SUCCESS) {
								return;
							}

							if (this.playerController.interactWithEntity(this.thePlayer, this.objectMouseOver.entityHit,
									this.thePlayer.getHeldItem(enumhand), enumhand) == EnumActionResult.SUCCESS) {
								return;
							}
							break;
						case BLOCK:
							...
							if (this.theWorld.getBlockState(blockpos).getMaterial() != Material.AIR) {
								...
								EnumActionResult enumactionresult = this.playerController.processRightClickBlock(
										this.thePlayer, this.theWorld, itemstack, blockpos,
										this.objectMouseOver.sideHit, this.objectMouseOver.hitVec, enumhand);
					...
					if (itemstack1 != null && this.playerController.processRightClick(this.thePlayer, this.theWorld,
							itemstack1, enumhand) == EnumActionResult.SUCCESS) {
						this.entityRenderer.itemRenderer.resetEquippedProgress(enumhand);
						return;
					}
				}
	...
	private boolean processKeyF3(int p_184122_1_) {
		if (p_184122_1_ == 30) {
			...
			this.debugChatMessage("RenderDistance: {0}",
					new Object[] { Integer.valueOf(this.gameSettings.renderDistanceChunks) });
			return true;
		} else if (p_184122_1_ == 35) {
			this.gameSettings.advancedItemTooltips = !this.gameSettings.advancedItemTooltips;
			this.debugChatMessage("Advanced tooltips: {0}",
					new Object[] { this.gameSettings.advancedItemTooltips ? "shown" : "hidden" });
			this.gameSettings.saveOptions();
			return true;
			...
			this.gameSettings.saveOptions();
			this.debugChatMessage("PauseOnLostFocus: {0}",
					new Object[] { this.gameSettings.pauseOnLostFocus ? "enabled" : "disabled" });
			return true;
					...
					if (this.thePlayer.isSpectator()) {
						...
						if (this.ingameGUI.getSpectatorGui().isMenuActive()) {
							...
							float f = MathHelper.clamp_float(
									this.thePlayer.capabilities.getFlySpeed() + (float) k * 0.005F, 0.0F, 0.2F);
	...
	public void launchIntegratedServer(String folderName, String worldName, @Nullable WorldSettings worldSettingsIn) {
		...
		networkmanager.sendPacket(new C00Handshake(110, socketaddress.toString(), 0, EnumConnectionState.LOGIN));
	...
	public void setDimensionAndSpawnPlayer(int dimension) {
		...
		this.thePlayer = this.playerController.createClientPlayer(this.theWorld,
				this.thePlayer == null ? new StatisticsManager() : this.thePlayer.getStatFileWriter());
	...
	private void middleClickMouse() {
		if (this.objectMouseOver != null && this.objectMouseOver.typeOfHit != RayTraceResult.Type.MISS) {
			...
			if (itemstack.getItem() == null) {
				String s1 = "";
				if (this.objectMouseOver.typeOfHit == RayTraceResult.Type.BLOCK) {
					s1 = ((ResourceLocation) Block.REGISTRY.getNameForObject(
							this.theWorld.getBlockState(this.objectMouseOver.getBlockPos()).getBlock())).toString();
				...
				if (flag) {
					...
					this.playerController.sendSlotPacket(this.thePlayer.getHeldItem(EnumHand.MAIN_HAND),
	...
	public CrashReport addGraphicsAndWorldToCrashReport(CrashReport theCrash) {
		...
		theCrash.getCategory().addCrashSection("CPU", new Callable<String>() {
			public String call() {
