...
=======
	...
	public void renderGameOverlay(float partialTicks, int pass, float distanceToB) {
		ScaledResolution scaledresolution = new ScaledResolution(this.mc);
		int i = scaledresolution.getScaledWidth();
		int j = scaledresolution.getScaledHeight();
		FontRenderer fontrenderer = this.getFontRenderer();
		...
		this.mc.getTextureManager().bindTexture(ICONS);
		GlStateManager.enableBlend();
		this.renderAttackIndicator(partialTicks, scaledresolution, pass, distanceToB);
		GlStateManager.tryBlendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA,
				GlStateManager.DestFactor.ONE_MINUS_SRC_ALPHA, GlStateManager.SourceFactor.ONE,
				GlStateManager.DestFactor.ZERO);
		...
		if (!this.mc.gameSettings.keyBindPlayerList.isKeyDown() || this.mc.isIntegratedServerRunning()
				&& this.mc.thePlayer.connection.getPlayerInfoMap().size() <= 1 && scoreobjective1 == null) {
			this.overlayPlayerList.updatePlayerList(false);
		} else {
			this.overlayPlayerList.updatePlayerList(true);
			this.overlayPlayerList.renderPlayerlist(i, scoreboard, scoreobjective1);
	...
	private void renderAttackIndicator(float p_184045_1_, ScaledResolution p_184045_2_, int pass, float distanceToB) {
		// distanceToB is a new parameter.
		GameSettings gamesettings = this.mc.gameSettings;
		if (gamesettings.thirdPersonView == 0) {
			if (this.mc.playerController.isSpectator() && this.mc.pointedEntity == null) {
				RayTraceResult raytraceresult = this.mc.objectMouseOver;
				...
				if (!this.mc.theWorld.getBlockState(blockpos).getBlock().hasTileEntity()
			...
			int l = p_184045_2_.getScaledWidth();
			int i1 = p_184045_2_.getScaledHeight();
			if (gamesettings.showDebugInfo && !gamesettings.hideGUI && !this.mc.thePlayer.hasReducedDebug()
			...
			for (PotionEffect potioneffect : Ordering.natural().reverse().sortedCopy(collection)) {
				...
				if (potion.hasStatusIcon() && potioneffect.doesShowParticles()) {
					...
					if (potioneffect.getIsAmbient()) {
						...
						if (potioneffect.getDuration() <= 200) {
							int j1 = 10 - potioneffect.getDuration() / 20;
							f = MathHelper.clamp_float((float) potioneffect.getDuration() / 10.0F / 5.0F * 0.5F, 0.0F,
									0.5F)
									+ MathHelper.cos((float) potioneffect.getDuration() * (float) Math.PI / 5.0F)
											* MathHelper.clamp_float((float) j1 / 10.0F * 0.25F, 0.0F, 0.25F);
						}
					}
	...
	protected void renderHotbar(ScaledResolution sr, float partialTicks) {
		...
		if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
			...
			this.drawTexturedModalRect(i - 91 - 1 + entityplayer.inventory.currentItem * 20,
					sr.getScaledHeight() - height - 1, 0, 22, 24, 22);
			if (itemstack != null) {
				if (enumhandside == EnumHandSide.LEFT) {
					this.drawTexturedModalRect(i - 91 - 29, sr.getScaledHeight() - height, 24, 22, 29, 24);
				} else {
